...
const ProfilePage = () => {
    ...
    const [errorsorganization, setErrosorganization] = React.useState<{ name: string }>();
    ...
    const checkforEmptyValue = (phone: any, name: any, dob: any, location: any, orgnization: any,
        pin: any, address: any, city: any) => {
        ...
        const input1 = document.getElementById('location') as HTMLInputElement | null;
        ...
        const input = document.getElementById('organization') as HTMLInputElement | null;
    ...
    const handleChangeInMobileNo = (event: React.ChangeEvent<HTMLInputElement>) => {
        setDisable(false)
    ...
    const handleChangeInFullName = (event: React.ChangeEvent<HTMLInputElement>) => {
        setDisable(false)
        const { target: { value } } = event;
        setErrosName({ name: '' });
        setName(value);
    ...
    return (
        <Grid container spacing={4} alignItems="center" justifyContent="center">
            <Grid item xs={12} md={7}>
                <Stack className = "divProfileFields" direction='row' spacing={5} sx={{ paddingLeft: "2%", paddingRight: "2%", paddingTop: "2%" }}>
                    {/* <Stack sx={{ width: '-webkit-fill-available' }}> */}
                
                        <Stack className='profileFields' >
                           
                                <Stack direction='column' spacing={3} sx={{ paddingTop: "1%" }} justifyContent="flex-start">
                                    <div>
                                        <Stack direction='row' spacing={2}>
                                            <TextField label='Full Name'
                                                ...
                                                helperText={errorsName?.name}></TextField>
                                    ...
                                    <Stack direction='row' spacing={2}>
                                        <TextField required id="organization" label='Organization Name' variant="outlined" value={organization}
                                            error={Boolean(errorsorganization?.name)}
                                            helperText={errorsorganization?.name}
                                        ></TextField>
                                        <TextField required id="location" label='Location' variant="outlined" value={location}
                                            error={Boolean(errorslocation?.name)}
                                            helperText={errorslocation?.name}
                                        ></TextField>
                                    </Stack>
                                    <TextField label='Address' variant="outlined" value={address} ></TextField>
                                    <Stack direction='row' spacing={2}>
                                        <TextField label='Pin' variant="outlined" value={pin} ></TextField>
                                        <TextField label='City' variant="outlined" value={city}></TextField>
                                    ...
                                    <Stack direction='row' spacing={2}>
                                        <TextField
                                            ...
                                            helperText={errors?.phone || errorsphone?.name}></TextField>

                                        <TextField label='Employee ID' value={'1234'} variant="outlined"
                                            helperText='Please enter full name'
                                            required aria-readonly></TextField>
                                    </Stack>
                                </Stack>
                          
                            <div className="savebutton">
                                <Button disabled={disable} variant="contained" color='primary'
                                    onClick={() => checkforEmptyValue(phone, name, dob, location,
                                        organization, pin, address, city)} >
                                    Save
                                </Button>
                            </div>
                                ...
                                <div className="card-header">Profile Picture
                                    ...
                                    <div className="card-body text-center">
                                        <div className="cardBodyItems">
                                            <div>
                                                <img className="img-account-profile rounded-circle mb-2" src="http://bootdey.com/img/Content/avatar/avatar7.png" alt=""></img>
                                            </div>
                                            <div>
